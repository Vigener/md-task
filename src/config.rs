...
fn get_config_search_paths() -> Vec<PathBuf> {
    ...
    if let Some(project_root) = find_project_root() {
        ...
        if project_config != paths[0] { // 重複回避
...
pub fn load_config() -> Config {
    ...
    for path in search_paths.iter().rev() { // 逆順で読み込み（後で上書き）
...
pub fn install_global_config() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !config_file.exists() {
        ...
        println!("Global config file already exists: {}", config_file.display());
    }
...
pub fn show_config_status() {
    ...
    println!("  Default priority: {}", config.task_management.default_priority);
